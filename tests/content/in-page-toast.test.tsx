...
describe("InPageToast", () => {
  ...
  it("applies hw-toast--info class for info variant", () => {
    const { container } = render(
      <InPageToast message="Test" locale="en" variant="info" onClose={() => {}} />
    );
  ...
  it("renders localized aria-labels when locale='uk'", () => {
    const { container } = render(
      <InPageToast message="Test" locale="uk" variant="hint" onClose={() => {}} />
    );
    expect(container.querySelector(".hw-toast__glyph")?.getAttribute("aria-label")).toBe(
      "Відкрити Word Hunter"
    );
  ...
  it("calls onClose when close button is clicked", () => {
    ...
    const { container } = render(
      <InPageToast message="Test" locale="en" variant="hint" onClose={onClose} />
    );
