...
- **`GameSettings` schema gained two booleans** (`autoContinue`, `showNextWordPreview`). Existing settings stores forward-fill via `DEFAULT_SETTINGS`, so no migration is needed. A subsequent change added four more booleans for `InPageNotifications` (`notificationsEnabled`, `showAutoModeToast`, `showHintToast`, `showNoParagraphToast`); the same forward-fill pattern applies.
