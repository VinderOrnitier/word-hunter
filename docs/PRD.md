...
13a. As a player, I want to see how many distinct words I've caught from each list (e.g. `34 / 68 Animals`) and a progress bar, so that I have a sense of overall completion.
...
- Tab "Play": **Hunt Collection** is the primary surface. Layout is a scrollable body above a non-scrolling `BottomActionBar`.
  ...
  - **CollectionGrid**: 4-column for Animals (68 slots), 5-column for Pokémon (151 slots). Each slot is a button — caught slots show emoji/sprite + `×N` counter; uncaught slots show `???` (Animals) or a `brightness(0)` silhouette (Pokémon). Clicking a slot marks it as `pending` (blue selection ring); pressing `Start a hunt` commits it to the `ActiveWord` (primary-yellow glow).
  - **BottomActionBar** (sticky, edge-to-edge): primary `Start a hunt` CTA (picks a random uncaught word from the active list, falling back to the full list if everything is caught), a `shuffle` icon button that does the same, a `pencil` icon button that opens the **CustomWordModal**, and a `↺` (refresh) icon toggle for **Auto-Continue** (`role="switch"`, `aria-label="Auto-continue"`, tooltip: "Auto-continue — pick next word after each find"). Off by default (`GameSettings.autoContinue = false`); when on, the icon uses a triple visual cue — primary-yellow color, border, and a soft outer ring. Custom-word hunts ignore the setting and always clear the active word on find.
...
- Tab "Settings": hint timer duration (minutes), hover duration for celebration tooltip (seconds), minimum paragraph word threshold (range slider 30–150, default 30), a `Show next word preview` switch (default on), a `Reload hint` switch (default on), and a **Notifications** section with a master `In-page notifications` toggle (default on) plus three individual switches beneath it — `Auto-Continue started`, `Hint reminder`, `No paragraphs` (all default on). The individual switches are visible but disabled when the master is off; their stored values are preserved so turning the master back on restores prior state.
