...
- Tab "Play": **Hunt Collection** is the primary surface. Layout is a scrollable body above a non-scrolling `BottomActionBar`.
  ...
  - **CollectionGrid**: 4-column for Animals (68 slots), 5-column for Pokémon (151 slots). Each slot is a button — caught slots show emoji/sprite + `×N` counter; uncaught slots show `???` (Animals) or a `brightness(0)` silhouette (Pokémon). Clicking a slot sets it as the `ActiveWord`; the matching slot gets a primary-yellow glow.
